...
public class VsGameFrame extends GameFrame implements ActionListener {
    private static final long serialVersionUID = 1L;

    public static final int singleBoardSize = 4;
    private SceneController sceneController;
    private JToggleButton kanjiButton[][] = new JToggleButton[singleBoardSize][singleBoardSize];
    private JLabel kanji[][] = new JLabel[singleBoardSize][singleBoardSize];
    private JTextField furigana;
    private JLabel selectLabel;
    private JButton answerButton;
    private ImageIcon defalutPanel, enablePanel;
    private int[][] board = new int[singleBoardSize][singleBoardSize];
    ...
    public VsGameFrame(SceneController sceneController) {
    ...
    protected void initVsGame() {
        ...
        layerePane.setPreferredSize(new Dimension(400, 500));

        ImagePanel backboardPanel = new ImagePanel("images/board.png");
        backboardPanel.setSize(new Dimension(400, 400));
        backboardPanel.setOpaque(false);
        layerePane.add(backboardPanel);
        layerePane.setLayer(backboardPanel, 30);

        JPanel boardPanel = new JPanel();
        boardPanel.setBounds(101, 106, 200, 200);
        boardPanel.setLayout(new GridLayout(4, 4));
        boardPanel.setOpaque(false);
        layerePane.add(boardPanel);
        layerePane.setLayer(boardPanel, 10);

        for (int i = 0; i < singleBoardSize; i++) {
            for (int j = 0; j < singleBoardSize; j++) {
        ...
        kanjiPanel.setBounds(106, 111, 200, 200);
        kanjiPanel.setLayout(new GridLayout(4, 4, 1, 1));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (searchSelect() == 0) {
            ...
            setPosition(x, y);
            selectLabel.setText(selectKanji);
        } else if (board[x][y] != CANSELECT && board[x][y] != SELECT) {
    ...
    public String selectedButton() {
        ...
        return getPosition() + "," + kanji + "," + furigana.getText();
    }
    ...
    public void highlight(int x, int y) {
        if (x == singleBoardSize - 1 && y == 0) {
            ...
            if (kanjiButton[x - 1][y].isSelected()) {
                ...
                board[x][y + 1] = CANSELECT;
                board[x - 1][y] = CANSELECT;
                board[x][y] = SELECT;
            }
        } else if (x == 0 && y == 0) {
            if (kanjiButton[x + 1][y].isSelected()) {
        ...
        } else if (x == singleBoardSize - 1 && y == singleBoardSize - 1) {
            if (kanjiButton[x - 1][y].isSelected()) {
                board[x][y] = SELECT;
                board[x - 1][y - 1] = BOARD;
                board[x - 2][y] = BOARD;
        ...
        } else if (x == 0 && y == singleBoardSize - 1) {
            if (kanjiButton[x + 1][y].isSelected()) {
                board[x][y] = SELECT;
                board[x + 1][y - 1] = BOARD;
                board[x + 2][y] = BOARD;
            ...
            public void actionPerformed(ActionEvent e) {
                if (timeLabel.getText().equals("0:00")) {
					...
					try {
                    	...
                    	sceneController.moveResult(2, kanjiData.getResultScore());
					}
